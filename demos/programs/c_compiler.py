    ...
    def compile_assignment(self, assign: Assignment) -> None:
        if isinstance(assign.target, VarRef):
            ...
            if isinstance(assign.target.index, IntLiteral):
                ...
                # Variable index -- dispatch tree for write
                src = self.compile_expr(assign.value, self.T0)
                idx_addr = self.compile_expr(assign.target.index, self.T1)
                end_label = self.new_label("arr_wr_end")
                for i in range(slot.size):
                    next_label = self.new_label("arr_wr") if i < slot.size - 1 else None
                    self.emit_mov(self.T2, idx_addr)
                    ci = self.mem.alloc_constant(i)
                    self.emit_sub(self.T2, ci)
                    if next_label:
                        self.emit_jnz(self.T2, next_label)
                    else:
                        self.emit_jnz(self.T2, end_label)
                    # Match: write
                    if assign.op == '=':
                        self.emit_mov(base + i, src)
                    elif assign.op == '+=':
                        self.emit_add(base + i, src)
                    elif assign.op == '-=':
                        self.emit_sub(base + i, src)
